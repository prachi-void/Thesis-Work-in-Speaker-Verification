...
def random_batch(speaker_num=config.N, utter_num=config.M, shuffle=True, noise_filenum=None, utter_start=0):
    ...
    if config.tdsv:
        ...
        utter_batch = np.concatenate([np.concatenate([utters[i]]*utter_num, axis=-
1) for i in range(speaker_num)], axis=1)
    ...
    # TI-SV
        ...
        np_file_list = [file for file in np_file_list if file.endswith('.npy')]
        ...
        utter_batch = np.concatenate(utter_batch, axis=2)     # utterance batch [batch(NM), n_mels, frames]
...
def similarity(embedded, w, b, N=config.N, M=config.M, P=config.proj, center=None):
    ...
    if center is None:
        ...
        S = tf.concat(
            [tf.concat([tf.reduce_sum(center[i:(i + 1), :] * embedded_split[j, :, :], axis=-1, keepdims=True) for i in range(N)],
                       axis=1) for j in range(N)], axis=0)
