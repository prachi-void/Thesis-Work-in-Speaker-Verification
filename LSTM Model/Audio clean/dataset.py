...
class CleanNoisyPairDataset(Dataset):
    ...
    def __init__(self, root='./', subset='training', crop_length_sec=0):
        ...
        #N_clean = os.listdir(os.path.join(root, 'clean_train'))
        N_clean = [file for file in os.listdir(os.path.join(root, 'clean_train')) if file.endswith('.wav')]
        N_noisy = os.listdir(os.path.join(root, 'Noisy_train_10_SNR'))
        #assert N_clean == N_noisy

        if subset == "training":
            cnt=226
           
            self.files = [(os.path.join(root, 'clean_train', i),
                         os.path.join(root, 'Noisy_train_10_SNR', i)) for i in N_clean]
            #self.files = [(os.path.join(root, 'clean_train', 'fileid_{}.wav'.format(i)),
             #              os.path.join(root, 'Noisy_train_10_SNR', 'fileid_{}.wav'.format(i))) for i in range(N_clean)]
        
        elif subset == "testing":
            sortkey = lambda name: '_'.join(name.split('_')[-2:])  # specific for dns due to test sample names
            #_p = os.path.join(root, 'clean_test')  # path for DNS
            
            clean_files = os.listdir(os.path.join(root, 'clean_test'))
            noisy_files = os.listdir(os.path.join(root, 'Noisy_test_10_SNR'))
            ...
            for _c, _n in zip(clean_files, noisy_files):
                ...
                self.files.append((os.path.join(root, 'clean_test', _c), 
                                   os.path.join(root, 'Noisy_test_10_SNR', _n)))
    ...
    def __getitem__(self, n):
        ...
        print((crop_length,len(clean_audio)))
...
if __name__ == '__main__':
    ...
    trainloader = load_CleanNoisyPairDataset(**trainset_config, subset='training', batch_size=2, num_gpus=0)
    testloader = load_CleanNoisyPairDataset(**trainset_config, subset='testing', batch_size=2, num_gpus=0)
