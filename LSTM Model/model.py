...
def train(path):
    ...
    with tf.variable_scope("lstm"):
        lstm_cells = [LSTMCell(units=config.hidden) for i in range(config.num_layer)]
        lstm = StackedRNNCells(lstm_cells)    # define lstm op and variables
    ...
    with tf.Session() as sess:
        ...
        for iter in range(config.iteration):
            ...
            if (iter+1) % 100 == 0:
                loss_values.append(loss_acc/100)
...
def test(path):
    ...
    with tf.variable_scope("lstm"):
        lstm_cells = [LSTMCell(units=config.hidden) for i in range(config.num_layer)]
        lstm = StackedRNNCells(lstm_cells)    # make lstm op and variables
        outputs, _ = tf.nn.dynamic_rnn(cell=lstm, inputs=batch, dtype=tf.float32, time_major=True)   # for TI-VS must use dynamic rnn
